...
bool BooleanExpressionHandler::checkAllTruthAssignments(const BooleanExpression* expression, bool expected)
{
	uint32_t usedCharacters = 0;
	expression->markUsedCharacters(usedCharacters);
 	size_t chCount = 0;
	for (size_t i = 0; i < ALLOWED_CHARACTERS_COUNT; i++)
	{
	...
	size_t truthAssignmentsCount = (1 << chCount);
	for (size_t i = 0; i < truthAssignmentsCount; i++)
	{
...
{
	return checkAllTruthAssignments(expression, true);
}

bool BooleanExpressionHandler::isContradiction() const
{
	return checkAllTruthAssignments(expression, false);
}
