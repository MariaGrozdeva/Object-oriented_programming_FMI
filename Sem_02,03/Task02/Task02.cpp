...
int getStudentsCount(const char* fileName)
{
...
size_t fillStudentsCollection(const char* fileName, StudentsCollection& collection)
{
	std::ifstream file(fileName);
	if (!file.is_open())
	{
	...
	size_t studentsCount = getStudentsCount(fileName);	
